...
export const MovieEdit = () => {
  const {formValues, onChange, errors, getMovie, updateMovie} = useContext(MovieContext)
  let {id} = useParams();
  ...
  return (
    <div className='mt-10'>
      <form onSubmit={updateMovie} className='max-w-md mx-auto p-5 bg-gray-100 shadow-sm rounded-md'>
        ...
        <div className='my-4 mx-auto flex justify-center'>
          <button className='px-4 py-2 bg-blue-500 hover:bg-blue-700 text-white rounded-md'>Update Movie</button>
        </div>
      </form>
    </div>
  )
